...
def find_col(df, exact_names=None, contains_all=None):
    ...
    for name in exact_names:
        if normalize_key(name) in normalized_map:
            return normalized_map[normalize_key(name)]
...
# ============================================================
# STRATEGIC INTELLIGENCE BUILDERS
# ============================================================
...
def build_conversation_starter(card):
    tags = [tag for tag in card.get("tags", []) if tag not in ["Existing Relationship", "Funding / Grants"]]
    district = card.get("name", "the district")

    if "Math" in tags and "MTSS" in tags:
        return f"For {district}, I’d start by asking: “As you work on math growth and intervention consistency, where do campuses need the most support turning data into daily instructional action?”"

    if "Curriculum / HQIM" in tags:
        return f"For {district}, I’d start by asking: “As adopted materials and instructional expectations move into classrooms, where are teachers needing the most practical implementation support?”"

    if "SPED/ELL" in tags:
        return f"For {district}, I’d start by asking: “Where are multilingual learners or students with disabilities needing more consistent access to grade-level expectations across campuses?”"

    if "CCMR" in tags:
        return f"For {district}, I’d start by asking: “Where are students first connecting academic skills to pathways, career readiness, and future opportunities?”"

    if "Math" in tags:
        return f"For {district}, I’d start by asking: “How are campuses currently using math data to decide which students need more practice, intervention, or targeted support?”"

    if tags:
        return f"For {district}, I’d start by asking: “How are these priorities showing up in day-to-day implementation across campuses?”"

    return f"For {district}, I’d start with discovery: “What district priorities are most difficult to translate into consistent campus-level practice right now?”"
...
def build_scoring_snapshot(card):
    return {
        ...
        "Score Band": card.get("score_band", ""),
        "Signal Density": card.get("signal_density", ""),
    }
...
def build_signal_density(tags, signals):
    ...
    if len(meaningful_tags) >= 5 or len(signals) >= 7:
        return "High"
...
def build_score_breakdown(card):
    ...
    if "Math" in tags:
        breakdown.append("Strategic fit driver: math improvement priority")
    if "MTSS" in tags:
        breakdown.append("Strategic fit driver: intervention / MTSS need")
    if "SPED/ELL" in tags:
        breakdown.append("Strategic fit driver: subgroup access and performance pressure")
    if "CCMR" in tags:
        breakdown.append("Strategic fit driver: college, career, and military readiness")
    if "Teacher Capacity" in tags:
        breakdown.append("Implementation driver: teacher capacity or professional learning")
    if "Curriculum / HQIM" in tags:
        breakdown.append("Implementation driver: curriculum or HQIM adoption fidelity")
    if "Funding / Grants" in tags:
        breakdown.append("Funding driver: grant or aligned funding signal")
    if "Existing Relationship" in tags:
        breakdown.append("Relationship driver: existing relationship or contract signal")
...
def build_docx(cards, data_source_label="Built-in workbook"):
    ...
    doc.add_paragraph(
        "Mobile conversation cards for conference prep: lead-with angle, strategic signals, contacts, PCG/Emerald alignment, scoring transparency, and discovery questions."
    )
    ...
    for idx, card in enumerate(cards):
        ...
        doc.add_heading("Conversation Starter", level=2)
        doc.add_paragraph(card.get("conversation_starter", ""))
...
def render_scoring_snapshot(card):
    ...
    st.markdown(f"- Score Band: {snapshot.get('Score Band', '')}")
    st.markdown(f"- Signal Density: {snapshot.get('Signal Density', '')}")
...
def render_card(card, data_source_label, view_mode="Quick Brief"):
    ...
    for item in card.get("next_moves", [])[:3]:
        st.markdown(f"- {item}")
    ...
    with st.expander("Copy Quick Prep", expanded=False):
        st.text_area("Quick prep copy", quick_prep, height=220, key=f"quick_{card['name']}")
...
def render_hot_accounts(cards):
    ...
    st.markdown(
        '<div class="helper-text">Top opportunities based on priority, score, signal density, and relationship context.</div>',
        unsafe_allow_html=True,
    )
...
def render_how_to(data_source_label):
    ...
    <ol>
      ...
      <li>Use Compare Districts or the full matrix download for deeper follow-up planning.</li>
    </ol>
    </div>
    """, unsafe_allow_html=True)

    st.markdown("### Useful searches")
    st.code("Dallas\nFort Worth\nSPED\nMTSS\nEureka\nBluebonnet\ncareer\nteacher burden\nTier 1", language="text")
